...
class ChatEnhancements {
  constructor(config = {}) {
    ...
    // Configurable wizard options (can be overridden via theme settings)
    this.wizardConfig = config.wizardConfig || {
      concerns: [
        { id: 'pain', label: 'Pain & Inflammation', icon: '💪' },
        { id: 'sleep', label: 'Sleep & Relaxation', icon: '😴' },
        { id: 'anxiety', label: 'Anxiety & Stress', icon: '🧘' },
        { id: 'recovery', label: 'Recovery & Wellness', icon: '🔄' },
        { id: 'focus', label: 'Focus & Clarity', icon: '🎯' },
        { id: 'general', label: 'General Wellness', icon: '✨' }
      ],
      applicationMethods: [
        { id: 'oil', label: 'CBD Oil/Tincture', icon: '💧', description: 'Fast-acting sublingual drops' },
        { id: 'gummies', label: 'Gummies/Edibles', icon: '🍬', description: 'Tasty, pre-dosed servings' },
        { id: 'topical', label: 'Topical/Cream', icon: '🧴', description: 'Targeted relief application' },
        { id: 'capsules', label: 'Capsules/Softgels', icon: '💊', description: 'Convenient daily supplement' },
        { id: 'vape', label: 'Vape/Inhale', icon: '💨', description: 'Fastest absorption method' },
        { id: 'pet', label: 'Pet Products', icon: '🐕', description: 'CBD for your furry friends' }
      ],
      strengths: [
        {
          id: 'low',
          label: 'Low Strength',
          icon: '🌱',
          description: '250mg - 500mg CBD',
          best_for: 'New users, mild symptoms, daily maintenance'
        },
        {
          id: 'medium',
          label: 'Medium Strength',
          icon: '⚖️',
          description: '500mg - 1500mg CBD',
          best_for: 'Regular users, moderate symptoms'
        },
        {
          id: 'high',
          label: 'High Strength',
          icon: '💪',
          description: '1500mg+ CBD',
          best_for: 'Experienced users, severe symptoms'
        }
      ],
      introMessage: "Let's find the perfect CBD product for you! I'll ask you a few quick questions.",
      concernTitle: 'What are you looking to address?',
      methodTitle: 'How would you like to take your CBD?',
      strengthTitle: 'What strength level do you need?'
    };
  ...
  startRecommendationWizard() {
    ...
    this.showMessage(this.wizardConfig.introMessage, 'bot');
  ...
  showConcernSelection() {
    const concerns = this.wizardConfig.concerns;

    const message = `
      <div class="wizard-step" style="margin: 8px 0;">
        <h4 style="margin: 0 0 12px 0; color: #333;">Step 1 of 3: ${this.escapeHtml(this.wizardConfig.concernTitle)}</h4>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 10px;">
          ${concerns.map(concern => `
            ...
            ">
              ...
              <span style="font-weight: 500; font-size: 14px;">${this.escapeHtml(concern.label)}</span>
            </button>
          `).join('')}
        </div>
      </div>
  ...
  showApplicationMethodSelection() {
    const methods = this.wizardConfig.applicationMethods;

    const message = `
      <div class="wizard-step" style="margin: 8px 0;">
        <h4 style="margin: 0 0 12px 0; color: #333;">Step 2 of 3: ${this.escapeHtml(this.wizardConfig.methodTitle)}</h4>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 10px;">
          ${methods.map(method => `
            ...
            ">
              <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                ...
                <span style="font-weight: 600; font-size: 14px;">${this.escapeHtml(method.label)}</span>
              </div>
              <p style="margin: 0; font-size: 12px; color: #666;">${this.escapeHtml(method.description)}</p>
            </button>
          `).join('')}
        </div>
      </div>
  ...
  showStrengthSelection() {
    const strengths = this.wizardConfig.strengths;

    const message = `
      <div class="wizard-step" style="margin: 8px 0;">
        <h4 style="margin: 0 0 12px 0; color: #333;">Step 3 of 3: ${this.escapeHtml(this.wizardConfig.strengthTitle)}</h4>
          ...
          ${strengths.map(strength => `
            ...
            ">
              <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                ...
                <span style="font-weight: 600; font-size: 16px;">${this.escapeHtml(strength.label)}</span>
              </div>
              <p style="margin: 0 0 4px 0; font-size: 13px; color: #444;">${this.escapeHtml(strength.description)}</p>
              <p style="margin: 0; font-size: 12px; color: #667eea; font-weight: 500;">Best for: ${this.escapeHtml(strength.best_for)}</p>
            </button>
          `).join('')}
        </div>
      </div>
  ...
  handleWizardSelection(action, value) {
    ...
    // Get label from wizard config dynamically
    let selectionLabel = value;
    if (action === 'concern') {
      const item = this.wizardConfig.concerns.find(c => c.id === value);
      selectionLabel = item ? item.label : value;
    } else if (action === 'applicationMethod') {
      const item = this.wizardConfig.applicationMethods.find(m => m.id === value);
      selectionLabel = item ? item.label : value;
    } else if (action === 'strength') {
      const item = this.wizardConfig.strengths.find(s => s.id === value);
      selectionLabel = item ? item.label : value;
    }
